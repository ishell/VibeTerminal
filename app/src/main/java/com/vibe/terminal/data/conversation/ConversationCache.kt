...
class ConversationCache @Inject constructor(
     ...
     * 获取元数据文件
     */
    private fun getMetadataFile(): File = File(cacheDir, "metadata.txt")
        ...
        try {
            ...
            if (metaFile.exists()) {
                metaFile.readLines().forEach { line ->
                    ...
                    if (parts.size == 2) {
                        metadataMap[parts[0]] = parts[1].toLongOrNull() ?: 0L
                    }
                }
            }
        } catch (_: Exception) {
    ...
    suspend fun isCacheValid(
        ...
        val cachedModTime = metadataMap[key] ?: 0L
        return cachedModTime >= remoteModTime
    }
